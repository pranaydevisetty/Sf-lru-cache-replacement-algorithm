#128K data cache and instruction cache with 64 bytes block for instruction cache and 64 bytes per block data cache
...
for i in range(len(addresses)):
    if addresses[i][0]==m1:
        ind.append(addresses_new[i][17:26])
        k=k+1
