#128K data cache and instruction cache with 4 bytes block for instruction cache and 4 bytes per block data cache
...
for i in range(len(addresses)):
    if addresses[i][0]==m1:
        ind.append(addresses_new[i][17:30])
        k=k+1
